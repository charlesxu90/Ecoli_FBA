...
# Usage:    python ./var_gene_cold.py cold cold_max/ 1246
##################################
suffix = sys.argv[1]
indir = sys.argv[2]
...
out_f = open('./var_cold_' + suffix + '.tsv', 'w')
err_f = open('./var_cold_' + suffix + '.tsv.err', 'w')
...
np.savetxt('cold_rxn_var.dat', mut_var, fmt='%10.3f', delimiter=',')
...
#             k = k + 1
#             frequency[i, k] = frequency[i, k] + 1
# np.savetxt('cold_frequency.dat', frequency, fmt='%i', delimiter=',')
