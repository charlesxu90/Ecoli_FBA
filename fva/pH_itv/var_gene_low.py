...
# Usage:    python ./var_gene_low.py low low_max/ 1246
##################################
suffix = sys.argv[1]
indir = sys.argv[2]
...
out_f = open('./var_low_' + suffix + '.tsv', 'w')
err_f = open('./var_low_' + suffix + '.tsv.err', 'w')
...
np.savetxt('low_rxn_var.dat', mut_var, fmt='%10.3f', delimiter=',')
...
#             k = k + 1
#             frequency[i, k] = frequency[i, k] + 1
# np.savetxt('low_frequency.dat', frequency, fmt='%i', delimiter=',')
