...
import hash_struct
import colSz2GRate
import gene2React2 as gene2React
##################################
#Author: xiaopeng xu
...
#cobra_model = suit_condition.gap_filling(cobra_model)
    ...
    try:
        ...
        for growthRate in Growth_Rates[geneId]:
            ...
            try:
                ...
                fva_file = open('./test/' + geneId +
                                TempRT[index] + '.txt', "w+")
                print('Reaction', 'Maximum flux', 'Minimum flux',
                ...
                for rxn in rxns_list:
                    ...
                    print (rxn.id, format(FVA_result[rxn.id]["maximum"], '.2f'),
                           format(FVA_result[rxn.id]["minimum"], '.2f'),
                           reactants, products,
                           file=fva_file, sep='\t')
                fva_file.close()
                index = index + 1
