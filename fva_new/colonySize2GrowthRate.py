...
with open(optgr_file, 'rb') as optgr_f:
    ...
    for row in tsvin:
        if np.float64(row[1]) < 8.57: # Genes with optGR equal to WT
            continue
...
print(len(optGR))
print(len(strainCSize))

NO_OF_GENES = len([k for k in strainCSize])    # No of unique genes in data
...
#optGrA = np.full((NO_OF_GENES, 1), 8.5712968149)
WT_gr = 8.5712968149

idx = []

i = 0
for gene in strainCSize:
    j = 0
    if gene in optGR:
        idx.append(i)
...
# GrowthRate = B * (X + C) = GRmax / (Xmax - Xmin) * ( X - Xmin)
...
for j in range(0, NO_OF_CONDITIONS):
    ...
    grs = WT_gr / (cmax[j] - cmin[j]) * (csizeC2G[:, j] - cminFc)
    #gr = np.minimum(grs, optGrA.transpose())
    # print("\t".join(map(str, grs[0, :]))) # OK
    print(conditions[j]  + "\t" + "\t".join(map(str, grs[0, idx])), file=gr_f)
    j += 1
gr_f.close()
